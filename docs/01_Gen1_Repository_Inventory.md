...
- CI copied into Gen2: **No**

The action column describes the **Gen2 artifact disposition**, not whether the underlying idea is valuable. `Retire` often means that a Gen1 path-coupled implementation artifact should not be copied; its concept may still be retained through a different Gen2 authority.
...
| Move | Concept remains but belongs under a different Gen2 taxonomy branch |
...
| Retire | Do not carry this artifact forward as a Gen2 artifact |
| New | Required Gen2 capability not represented by a sufficient Gen1 artifact |

## 3. Complete Gen1 File Inventory

| # | Path | Lines | Gen1 Version | Gen1 Status | Role | Gen2 Action | Reason |
|---:|---|---:|---|---|---|---|---|
...
| 3 | `.github/REPOSITORY_PROTECTION.md` | 44 | — | — | External trust / release protection requirements | **Move** | Move concepts to Gen2 Governance; do not copy path-specific controls yet. |
| 4 | `.github/workflows/document-validation.yml` | 36 | — | — | GitHub Actions validation workflow | **Retire** | No CI in v0.0.1rc1; future tooling must follow stable Gen2 contracts. |
| 5 | `CHANGELOG.md` | 271 | — | — | Gen1 revision history | **Keep** | Preserve as historical source only; Gen2 has its own changelog. |
| 6 | `CONTRIBUTING.md` | 37 | — | — | Contribution and IP policy | **Rewrite** | Legal/governance intent may remain, but Gen2 repository scope and contribution model must be re-established. |
| 7 | `LICENSE` | 37 | — | — | Repository legal terms | **Keep** | Carried into Gen2 analysis RC unless separately changed. |
| 8 | `NOTICE.md` | 63 | — | — | Copyright, disclaimer, AI and third-party notice | **Rewrite** | Preserve legal concepts, rewrite against Gen2 identity and final governance structure. |
| 9 | `README.md` | 277 | — | — | Gen1 repository entry point and document routing | **Rewrite** | Gen2 requires a new system-level entry point and taxonomy. |
| 10 | `authority-registry.yaml` | 529 | — | — | Machine-readable document authority and prerequisite registry | **Merge** | Authority-registry concept is strong; rebuild after Gen2 topic ownership converges. |
...
| 13 | `docs/coding-rules/README.md` | 18 | — | — | Directory index / authority-routing summary | **Retire** | Gen2 will use a new taxonomy and new navigation structure. |
...
| 20 | `docs/coordinator/README.md` | 20 | — | — | Coordinator document index / authority summary | **Retire** | Coordinator is no longer a top-level Gen2 document domain. |
...
| 23 | `docs/framework/Framework_Application_Analysis_Template.md` | 2849 | v1.1.9 | Baseline | Normative framework-application analysis method | **Rewrite** | Evolve into Gen2 Framework Scan / Applicability Analysis and project decision/evidence model. |
| 24 | `docs/framework/README.md` | 15 | — | — | Framework document index / authority summary | **Retire** | Replaced by the Gen2 repository entry point and system-level taxonomy. |
...
| 39 | `docs/validation/Validation_Evidence_Guide.md` | 389 | v1.1.0 | Draft for Review | Proposed operational validation-evidence method; not a Product, architecture, Protocol, role, or language authority | **Merge** | Elevate evidence model into Gen2 Verification & Evidence, integrated with incident evidence and applicability decisions. |
| 40 | `examples/framework-conformance-claim.yaml` | 56 | — | — | Machine-readable conformance claim example | **Rewrite** | Concept remains; data model must reflect Gen2 applicability and evidence taxonomy. |
...
| 44 | `schema/protocol.schema.yaml` | 223 | — | — | Executable protocol schema subset | **Rewrite** | Protocol contract remains, but schema must align with Gen2 interface taxonomy. |
| 45 | `tests/fixtures/protocol/invalid_address_reuse_session.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 46 | `tests/fixtures/protocol/invalid_boolean_string.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 47 | `tests/fixtures/protocol/invalid_bounded_parallel_update.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 48 | `tests/fixtures/protocol/invalid_broadcast_response_policy.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 49 | `tests/fixtures/protocol/invalid_explicit_blank_core_profile.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 50 | `tests/fixtures/protocol/invalid_explicit_empty_core_profile.yaml` | 46 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 51 | `tests/fixtures/protocol/invalid_group_session_without_profile.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 52 | `tests/fixtures/protocol/invalid_identity_conflict_policy.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 53 | `tests/fixtures/protocol/invalid_legacy_empty_profile.yaml` | 13 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 54 | `tests/fixtures/protocol/invalid_maximum_nodes_zero.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 55 | `tests/fixtures/protocol/invalid_multi_target_partial_failure.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 56 | `tests/fixtures/protocol/invalid_scope_typo.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 57 | `tests/fixtures/protocol/invalid_shared_bus_without_addressing.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 58 | `tests/fixtures/protocol/valid_legacy_single_node.yaml` | 24 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 59 | `tests/fixtures/protocol/valid_multi_independent.yaml` | 58 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 60 | `tests/fixtures/protocol/valid_multi_shared_bus.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 61 | `tests/fixtures/protocol/valid_routed_gateway.yaml` | 58 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 62 | `tests/fixtures/protocol/valid_single_node.yaml` | 57 | — | — | Positive/negative protocol semantic-validation fixture | **Retire** | Do not migrate fixtures in rc1; preserve scenario intent for future machine-verifiable protocol tests. |
| 63 | `tests/fixtures/protocol_expectations.yaml` | 38 | — | — | Expected validator outcomes for protocol fixtures | **Retire** | Tool/test implementation deferred until Gen2 schemas stabilize. |
...
| 70 | `tools/validate_protocol.py` | 648 | — | — | Gen1 repository/protocol/external-anchor validation tool | **Retire** | Tool implementation is path/schema coupled; rebuild later from stable Gen2 contracts. |
| 71 | `tools/validate_repository.py` | 1674 | — | — | Gen1 repository/protocol/external-anchor validation tool | **Retire** | Tool implementation is path/schema coupled; rebuild later from stable Gen2 contracts. |
| 72 | `tools/verify_external_anchor.py` | 246 | — | — | Gen1 repository/protocol/external-anchor validation tool | **Retire** | Tool implementation is path/schema coupled; rebuild later from stable Gen2 contracts. |


## 4. Supplemental Source Inventory

| # | Path | Lines | Version / Status | Role | Gen2 Action |
|---:|---|---:|---|---|---|
| 1 | `README.md` | 4954 | v1.0.1rc03 / Specification RC | Incident evidence, first-abnormal-state, survivability, persistence, recovery, recorder self-protection, observer-effect and implementation contract | **Merge** into Gen2 Robustness & Resilience plus Verification & Evidence; keep recorder implementation details subordinate |
| 2 | `LICENSE` | legal text | — | Legal terms | **Reference**; Gen2 license currently follows Gen1 terms |
...
Gen2 should extract and re-home the durable concepts rather than reproducing the same folder graph with broader names.
