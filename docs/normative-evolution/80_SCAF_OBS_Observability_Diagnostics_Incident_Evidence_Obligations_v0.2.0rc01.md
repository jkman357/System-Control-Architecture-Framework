...
**Candidate-overlay note:** this file is the complete proposed `SCAF-OBS` replacement for `v0.2.0rc01`. The formal frozen source remains `docs/normative/80_SCAF_OBS_Observability_Diagnostics_Incident_Evidence_Obligations.md` until a later explicit promotion decision. Existing IDs `SCAF-OBS-001` through `SCAF-OBS-040` retain their frozen identity; candidate IDs `SCAF-OBS-041` through `SCAF-OBS-045` are not yet part of the frozen machine-readable authority registry.
