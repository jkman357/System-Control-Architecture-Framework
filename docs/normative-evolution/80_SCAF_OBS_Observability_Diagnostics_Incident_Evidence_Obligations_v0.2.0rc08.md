...
**Candidate Release:** v0.2.0rc08  
**Formal Predecessor:** frozen v0.1.0 using v0.0.2 canonical OBS authority  
**Concern:** `SCAF-OBS`  
**Layer:** L1 Concern Authority + L2 Required Project Decisions  
**Status:** Candidate normative overlay; independent review required before canonical promotion

## 1. Purpose

**Candidate-overlay note:** this file is the complete proposed `SCAF-OBS` replacement for `v0.2.0rc08`. The formal frozen source remains `docs/normative/80_SCAF_OBS_Observability_Diagnostics_Incident_Evidence_Obligations.md` until a later explicit promotion decision. Existing IDs `SCAF-OBS-001` through `SCAF-OBS-040` retain their frozen identity. Accepted development-line candidate IDs `SCAF-OBS-041` through `SCAF-OBS-045` retain the exact rc01 candidate wording. This rc08 overlay adds only candidate IDs `SCAF-OBS-046` through `SCAF-OBS-048`. The current machine-readable candidate authority registry remains separately bound to the accepted rc01 overlay and therefore still represents only `SCAF-OBS-041..045` until a later reviewed representation migration is justified.
...
- diagnostic-instrumentation lifecycle intent, including explicit distinction between retained observation and development-scoped instrumentation where material;
- development-scoped instrumentation cleanup/retention disposition and instrumented-build evidence identity where material;
- operational non-dependence/failure-isolation expectations for observation paths that are not themselves required source functionality;
- evidence-to-realization applicability binding where source/build/configuration/target/instrumentation identity materially affects interpretation;
- controlled baseline/change relationships where diagnosis, regression evaluation or verification materially depends on comparison between realizations;
- controlled before/after evidence attribution for verification/closure comparisons without transferring evidence-sufficiency or underlying closure authority;
...
Where project-added or materially changed observation/diagnostic instrumentation has a lifecycle consequence, the project **SHALL** define whether that instrumentation is intended to remain beyond the current development activity or is development-scoped instrumentation introduced for a bounded engineering purpose.

The project need not use universal labels such as monitor, probe, trace point or breadcrumb. The controlled lifecycle intent and diagnostic purpose are normative; local naming and implementation mechanism are not.

### `SCAF-OBS-042` — Development-scoped instrumentation purpose and removal criterion

**Target:** Project-Applicable Obligation

For each material development-scoped instrumentation item, the project **SHALL** define the bounded engineering question/purpose that justifies the instrumentation and the expected removal point, removal criterion or other controlled disposition trigger.

Development-scoped instrumentation **SHALL NOT** become retained diagnostic capability solely because it is inexpensive, disabled in one build profile or left in the source after its original purpose is complete.

### `SCAF-OBS-043` — Development instrumentation closure disposition

**Target:** Project-Applicable Obligation

At closure of the feature, defect, refactoring, timing study or verification activity that introduced or materially changed development-scoped instrumentation, the project **SHALL** explicitly disposition each such material item as either removed or intentionally retained/promoted as ongoing diagnostic instrumentation.

Intentional retention/promotion **SHALL** establish a continuing diagnostic purpose and **SHALL** re-evaluate applicable observer-effect, resource/capacity, evidence-volume, failure-isolation, retention/accessibility and verification consequences under `SCAF-OBS`, `SCAF-TIME`, `SCAF-ROB` and other applicable source concerns.

A build-profile exclusion or disabled compile switch by itself **SHALL NOT** be treated as evidence of removal when the project closure decision is that the temporary instrumentation should no longer remain in the maintained source baseline.

### `SCAF-OBS-044` — Instrumented-build evidence identity and cleanup re-evaluation

**Target:** Project-Applicable Obligation

Where temporary instrumentation can materially affect behavior, timing, capacity, memory layout, scheduling, evidence production or another verification-relevant property, evidence obtained from the instrumented build **SHALL** preserve sufficient build/configuration/instrumentation identity to prevent silent attribution of that evidence to a materially different cleaned build.

After instrumentation removal, retention/promotion or other material instrumentation change, the project **SHALL** determine and perform the regression, timing/resource confirmation or other verification re-evaluation required by the affected source obligations and project governance. This obligation does not prescribe a universal test depth for every cleanup change.

### `SCAF-OBS-045` — Observation-path operational non-dependence and retained-cost acceptance

**Target:** Project-Applicable Obligation

Where an observation, diagnostic persistence or evidence-export path is not itself required source Function/Service behavior, the project **SHALL** define the architecture consequence so loss, saturation, delay or failure of that observation path degrades evidence availability/quality rather than silently becoming a prerequisite for the source Function/Service to continue correctly. Any intentional functional dependency on an observation path **SHALL** be established by the applicable source authority rather than inferred from the diagnostic realization.

For instrumentation intentionally retained beyond development, the project **SHALL** define and accept the applicable bounded observer-effect and resource/capacity contribution using `SCAF-OBS-013`, `SCAF-TIME` and `SCAF-ROB` as applicable. This requirement does not impose universal zero overhead or prescribe asynchronous buffering, storage media, task priority or transport mechanism.

### `SCAF-OBS-046` — Evidence realization applicability binding

**Target:** Project-Applicable Obligation

Where interpretation, diagnosis, regression evaluation or verification applicability of material evidence depends on the realized source revision, build, configuration, target/hardware revision, instrumentation configuration, operational/lifecycle identity or another source-owned realization identity, the project **SHALL** preserve or associate sufficient controlled identity/provenance to determine whether that evidence applies to the realization being analyzed or verified.

Material evidence **SHALL NOT** be silently attributed or applied to a materially different realization context when the difference can invalidate interpretation. This obligation does not require every evidence item to carry every possible identity field. The applicable source concerns continue to define the meaning of source/build/configuration/target/runtime/lifecycle identities; `SCAF-OBS` associates evidence with those identities without redefining them.

### `SCAF-OBS-047` — Baseline and change relationship for comparative evidence

**Target:** Project-Applicable Obligation

Where diagnosis, regression evaluation or verification materially depends on comparison to a prior, reference or known-good realization, the project **SHALL** identify the controlled baseline and the relevant change identity/scope sufficiently to establish which realization states and change boundary form the comparative evidence basis. Evidence used by the comparison **SHALL** remain attributable to the applicable baseline and changed realization contexts where that attribution is material to interpretation.

Membership in, proximity to or temporal association with the relevant change set **SHALL NOT** by itself establish causality. Causal or derived-inference claims remain governed by `SCAF-OBS-009`. This obligation does not require Git, commit hashes, patch files or a particular source-control/change-management mechanism.

### `SCAF-OBS-048` — Before/after evidence relationship for verification and closure

**Target:** Project-Applicable Obligation

Where verification or closure materially relies on comparison of behavior before and after a change, correction or fix, the project **SHALL** preserve the controlled relationship and realization applicability of the compared evidence sufficiently to distinguish the baseline/prior condition from the changed/corrected condition and to evaluate the applicable intended-behavior and regression properties identified by the Applicable Satisfaction Basis.

The existence of before/after evidence **SHALL NOT** by itself establish evidence sufficiency or underlying closure. Evidence-sufficiency evaluation remains with Project Verification / Assurance Authority under `SCAF-AK-007`; underlying closure authority remains with the applicable owner under `SCAF-AK-012`, and closure/disposition trace remains subject to `SCAF-AK-013`. This obligation does not require runtime logging or a universal before/after test procedure when another controlled verification basis is appropriate.
...
| Diagnostic instrumentation lifecycle intent and development-scoped purpose/removal criterion | Project Design Authority under OBS obligations and project lifecycle governance |
| Development instrumentation remove-or-retain disposition | Project Design Authority / project change-closure governance using applicable OBS/TIME/ROB/ASSUR inputs |
| Instrumented-build identity and cleanup re-evaluation | Project Design Authority + Project Verification / Assurance Authority using affected source obligations |
| Observation-path operational non-dependence / retained-cost acceptance | Project Design Authority using source Function/Service authority plus OBS/TIME/ROB constraints |
...
During development, a project may temporarily add execution markers around a changed UI or protocol path to answer a timing/behavior question. Those markers are development-scoped instrumentation. After evidence collection, each is removed or intentionally retained after continuing diagnostic value and runtime cost are accepted. A retained queue-loss counter may remain as operational diagnostic evidence while the temporary timing markers are removed. The cleaned build then receives the regression/timing confirmation required by the affected project obligations; SCAF does not prescribe the concrete logger, storage task, SD/USB path or test procedure.
